...
@testset "Migrations" begin
    @testset "SQLite" begin
        ...
        dir = "test1"
        migrations = Migrations.runmigrations(db, abspath(joinpath(dirname(pathof(Migrations)), "..", "test/sqlite", dir)))
        ...
        dbmigrations = [Migrations.Migration(m.filename, m.sha256, m.applied_at) for m in DBInterface.execute(db, "SELECT * FROM __migrations__")]
        ...
        dir = "error1"
        @test_throws SQLiteException Migrations.runmigrations(db, abspath(joinpath(dirname(pathof(Migrations)), "..", "test/sqlite", dir)))
        ...
        dir = "error2"
        @test_throws Migrations.DuplicateMigrationError Migrations.runmigrations(db, abspath(joinpath(dirname(pathof(Migrations)), "..", "test/sqlite", dir)))
    end
end
